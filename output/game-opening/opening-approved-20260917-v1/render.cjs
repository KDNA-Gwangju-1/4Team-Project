...
(async()=>{
 ...
 for(let i=0;i<N;i++){
  ...
  if(i<144){
   ...
   const sk=ease((st-3.6)/.4);if(sk>0){g.globalAlpha=sk;const drift=clamp((st-4)/12);g.drawImage(sky,lerp(0,sky.width*.018,drift),sky.height*.005,sky.width*.975,sky.height*.975,0,0,W,H);g.globalAlpha=1;}
