...
(async()=>{
 ...
 const output=path.join(OUT,'opening-final-1080p.mp4'),audio=path.join(OLD,'opening-restored-final-1080p.mp4');
 let p,done,errors='';
 if(!preview){p=spawn(FF,['-v','error','-y','-f','rawvideo','-pix_fmt','rgba','-s','1920x1080','-r','30','-i','pipe:0','-i',audio,'-map','0:v','-map','1:a:0','-c:v','libx264','-preset','fast','-crf','18','-pix_fmt','yuv420p','-c:a','copy','-t','33.3','-movflags','+faststart',output],{windowsHide:true});p.stderr.on('data',x=>errors+=x);p.stdin.on('error',()=>{});done=new Promise((r,j)=>{p.on('error',j);p.on('close',n=>n?j(Error(errors)):r());});done.catch(()=>{});}
 const shots=[0,54,75,90,105,120,129,135,143,144,234,270,323,328,335,429,449,470,490,515,535,552,660,765,840,890,965];
 const camera=[];
 for(let i=0;i<N;i++){
  ...
  if(i<144){
   ...
   const q=ease((t-1.8)/2.5),cw=W/Math.exp(Math.log(W/270)*q),ch=cw*H/W;
   const x=1520*(W-cw)/(W-270),y=602*(H-ch)/(H-270*H/W),s=W/cw;
   ...
   const dx=(1520-x)*s,dy=(602-y)*s,dw=270*s,dh=270*H/W*s;
   // Measured glass: x1520..1790, y598..758. Preserve rounded physical
   // glass independently of the 16:9 programme; extend only edge scanlines.
   g.save();g.beginPath();g.roundRect(dx,(598-y)*s,dw,160*s,8*s);g.clip();
   g.drawImage(news,0,0,news.width,1,dx,(598-y)*s,dw,4*s);
   g.drawImage(news,0,news.height-1,news.width,1,dx,dy+dh,dw,5*s);
   g.drawImage(news,dx,dy,dw,dh);g.restore();
   // Remove the glass corner occlusion only as the bezel exits the camera.
   // This uses the SAME content transform, never a resized replacement shot.
   if(q>.96){g.save();g.globalAlpha=ease((q-.96)/.04);g.drawImage(news,dx,dy,dw,dh);g.restore();}
   ...
   const st=t-14.3,q=ease((st-.7)/3.3),cw=lerp(.60,.32,q)*env.width,x=lerp(.40,.003,q)*env.width,y=lerp(.34,.002,q)*env.height;
   g.drawImage(env,x,y,cw,cw*H/W,0,0,W,H);
   // Constant-speed sky/cloud drift, including ringing, pickup and dialogue.
   // Overscan supplies real offscreen pixels without looping seams or empty edges.
   const sk=ease((st-3.6)/.4);if(sk>0){g.globalAlpha=sk;const drift=clamp((st-3.6)/12.4);g.drawImage(sky,lerp(0,sky.width*.13,drift),sky.height*.04,sky.width*.87,sky.height*.87,0,0,W,H);g.globalAlpha=1;}
 ...
 const bytes=await fs.readFile(output);await fs.writeFile(path.join(OUT,'manifest.json'),JSON.stringify({output,size:[W,H],fps:FPS,frames:N,duration:33.3,bytes:bytes.length,sha256:crypto.createHash('sha256').update(bytes).digest('hex'),audio:'Existing full AAC stream copied without remix or re-encoding',integration:'Authorized 2026-09-17: original daytime lobby, corrected leaf-window pose, news fixes; previous final untouched',changes:['One monotonic lobby camera; fixed-size news layout; full-screen hold at 4.3s','Expert caption: one line, 48px','Window: foreground tree retained, both hands on window, upper-left wall filled; camera environment outpainted for pan to sky'],source:audio,hospitalIncluded:false,cameraChecks:'Monotonic screen width; exact 1920px endpoint',caption:expertText},null,2));console.log(output);
})();
