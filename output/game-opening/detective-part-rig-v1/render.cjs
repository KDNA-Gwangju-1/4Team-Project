...
function drawRig(g,t,x,y,scale,debug=false){const j=pose(t);g.save();g.translate(x,y);g.scale(scale,scale);g.globalAlpha=1;g.imageSmoothingEnabled=false;
...
(async()=>{GlobalFonts.registerFromPath('C:/Windows/Fonts/malgun.ttf','Korean');const atlas=await loadImage(path.join(OUT,'parts-atlas.png')),ac=C(atlas.width,atlas.height),ag=ac.getContext('2d');ag.drawImage(atlas,0,0);const px=ag.getImageData(0,0,atlas.width,atlas.height).data,meta={};
