...
(async()=>{
 ...
 const cycle=[],cycleRigs=[];for(let i=0;i<N;i++){const pos=i/N*4,k=Math.floor(pos),t=pos-k,j=(k+1)%4,target=rigs[k].map((p,n)=>p.map((v,l)=>lerp(v,rigs[j][n][l],t)));cycleRigs.push(target);cycle.push(blend(warp(key[k],rigs[k],target),warp(key[j],rigs[j],target),t));if(i%18===0)console.log('Inbetween',i,'/',N);}
 ...
 const tween=(a,ar,b,br,q)=>{const target=ar.map((p,n)=>p.map((v,l)=>lerp(v,br[n][l],q)));return blend(warp(a,ar,target),warp(b,br,target),q);};
