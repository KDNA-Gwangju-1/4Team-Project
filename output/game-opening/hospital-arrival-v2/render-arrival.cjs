...
const FPS=30,N=36,S=512,W=1920,H=1080,D=11.6;
...
function warp(img,from,to){const c=canvas(S,S),g=c.getContext('2d');g.imageSmoothingEnabled=false;const step=64;
 for(let y=0;y<S;y+=step)for(let x=0;x<S;x+=step){const a=[[x,y],[x+step,y],[x+step,y+step],[x,y+step]],b=a.map(p=>displaced(...p,from,to));triangle(g,img,[a[0],a[1],a[2]],[b[0],b[1],b[2]]);triangle(g,img,[a[0],a[2],a[3]],[b[0],b[2],b[3]]);}return c;}
function blend(a,b,t){const c=canvas(S,S),g=c.getContext('2d'),aa=a.getContext('2d').getImageData(0,0,S,S).data,bb=b.getContext('2d').getImageData(0,0,S,S).data,o=g.createImageData(S,S),d=o.data;for(let i=0;i<d.length;i+=4){let wa=aa[i+3]*(1-t),wb=bb[i+3]*t,al=wa+wb;d[i+3]=al;if(al)for(let k=0;k<3;k++)d[i+k]=(aa[i+k]*wa+bb[i+k]*wb)/al;}g.putImageData(o,0,0);return c;}
...
(async()=>{
 ...
 const cycle=[];for(let i=0;i<N;i++){const pos=i/N*4,k=Math.floor(pos),t=pos-k,j=(k+1)%4,target=rigs[k].map((p,n)=>p.map((v,l)=>lerp(v,rigs[j][n][l],t)));cycle.push(blend(warp(key[k],rigs[k],target),warp(key[j],rigs[j],target),t));if(i%9===0)console.log('Inbetween',i,'/',N);}
 ...
 await encode('hospital-entry-1080p.mp4',D,(g,t)=>{
  ...
  if(t<8.25){if(open>0)door(g,open);let walk=clamp((t-.7)/6.2),z=lerp(2.2,7.8,walk),h=2000/z,foot=920+800/z+(1-smooth((t-.7)/.55))*1050,x=lerp(960,928,smooth(walk));
   let sprite=cycle[Math.floor(Math.max(0,t-.7)*FPS)%N];if(t>=6.9){h=2000/7.8;foot=920+800/7.8;x=928;sprite=blend(cycle[6],reach,smooth((t-6.9)/.55));}
   if(t>=7.65){const q=smooth((t-7.65)/.6);x=lerp(928,1024,q);sprite=blend(reach,cycle[Math.floor((t-7.65)*FPS)%N],smooth((t-7.65)/.28));}
   ...
   g.save();polygon(g,full);g.clip();actor(g,cycle[Math.floor((t-8.25)*FPS)%N],x,foot,h,.48*smooth(q));g.restore();if(open>0)door(g,open);else g.drawImage(closed,0,0);
  }
 },true);
