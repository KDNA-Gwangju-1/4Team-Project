...
function warp(img,from,to){
 ...
 // Keep every mesh triangle orientation positive. Folded triangles can paint
 // the same body region twice even when all pixels are opaque.
 const step=16,tri=[];
 for(let y=0;y<S;y+=step)for(let x=0;x<S;x+=step){const a=[[x,y],[x+step,y],[x+step,y+step],[x,y+step]],b=a.map(p=>displaced(...p,from,to));for(const ix of [[0,1,2],[0,2,3]])tri.push({a:ix.map(i=>a[i]),b:ix.map(i=>b[i])});}
 const area=p=>(p[1][0]-p[0][0])*(p[2][1]-p[0][1])-(p[1][1]-p[0][1])*(p[2][0]-p[0][0]);
 let strength=1,minRatio=-1;
 while(true){minRatio=Math.min(...tri.map(({a,b})=>area(a.map((p,i)=>p.map((v,k)=>lerp(v,b[i][k],strength))))/(step*step)));if(minRatio>=.25)break;strength*=.9;if(strength<.01)throw Error('Cannot obtain nonfolding mesh');}
 for(const {a,b} of tri)raster(a,a.map((p,i)=>p.map((v,k)=>lerp(v,b[i][k],strength))));
 let fractionalAlpha=0;for(let i=3;i<d.length;i+=4)if(d[i]!==0&&d[i]!==255)fractionalAlpha++;
 if(fractionalAlpha)throw Error('Translucent pose pixels found');
 deformationAudit.push({strength,minTriangleAreaRatio:minRatio,fractionalAlpha});
...
async function encode(name,duration,draw,sound){const c=canvas(W,H),g=c.getContext('2d');const args=['-y','-v','error','-f','rawvideo','-pix_fmt','rgba','-s',`${W}x${H}`,'-r',String(FPS),'-i','pipe:0'];if(sound)args.push('-i',path.join(OUT,'arrival-foley.wav'));args.push('-c:v','libx264','-crf','18','-preset','fast','-pix_fmt','yuv420p');if(sound)args.push('-c:a','aac','-b:a','160k');args.push('-t',String(duration),'-movflags','+faststart',path.join(OUT,name));const p=spawn(FF,args,{windowsHide:true});let err='';p.stderr.on('data',b=>err+=b);p.stdin.on('error',()=>{});const done=new Promise((r,j)=>{p.on('error',j);p.on('close',n=>n?j(Error(err)):r());});done.catch(()=>{});
 for(let i=0;i<duration*FPS;i++){g.setTransform(1,0,0,1,0,0);g.globalAlpha=1;g.globalCompositeOperation='source-over';g.clearRect(0,0,W,H);draw(g,i/FPS);if(sound&&[30,65,150,213,243,270,330,414,432,450,480,495,540,600].includes(i))await fs.writeFile(path.join(OUT,`frame-${i}.png`),c.toBuffer('image/png'));if(!p.stdin.write(Buffer.from(g.getImageData(0,0,W,H).data)))await once(p.stdin,'drain');}p.stdin.end();await done;}
(async()=>{
 ...
 function actor(g,sprite,x,foot,h,dark=0){const f=h/422,dx=Math.round(x-256*f),dy=Math.round(foot-459*f);g.save();g.globalAlpha=1;g.imageSmoothingEnabled=false;let source=sprite;if(dark){const shade=canvas(S,S),sg=shade.getContext('2d');sg.drawImage(sprite,0,0);sg.globalCompositeOperation='source-atop';sg.fillStyle=`rgba(20,30,45,${dark})`;sg.fillRect(0,0,S,S);source=shade;}g.drawImage(source,dx,dy,Math.round(512*f),Math.round(512*f));g.restore();}
 await audio();
 await fs.writeFile(path.join(OUT,'deformation-audit.json'),JSON.stringify({poseCount:deformationAudit.length,allAlphaBinary:deformationAudit.every(x=>x.fractionalAlpha===0),allTrianglesUnfolded:deformationAudit.every(x=>x.minTriangleAreaRatio>=.25),poses:deformationAudit},null,2));
