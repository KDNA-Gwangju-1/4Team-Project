...
const W=1672,H=941,FPS=60/7,DUR=12.6,CAM={x:1045,y:407,f:900,h:1.15},HUMAN=1.70;
...
(async()=>{
 ...
 const dir=path.join(O,'keys');fs.mkdirSync(dir,{recursive:true});
 ...
 for(let n=0;n<Math.round(DUR*FPS);n++){
  const t=n/FPS,s=state(t),ci=checks.findIndex(v=>Math.round(v*FPS)===n);if(stills&&ci<0)continue;
 ...
 fs.writeFileSync(path.join(O,'motion.json'),JSON.stringify({camera:CAM,doorDepth:DZ,fps:FPS,duration:DUR,frames:audit},null,2));
 const strip=createCanvas(1600,Math.ceil(checks.length/4)*250),cg=strip.getContext('2d');cg.fillStyle='#1b1e23';cg.fillRect(0,0,strip.width,strip.height);
 for(let i=0;i<checks.length;i++){const im=await loadImage(path.join(O,`check-${String(i).padStart(2,'0')}.png`));const x=i%4*400,y=Math.floor(i/4)*250;cg.drawImage(im,x,y,400,225);cg.fillStyle='#fff';cg.font='18px sans-serif';cg.fillText(`${checks[i].toFixed(2)} s`,x+8,y+244);}
 fs.writeFileSync(path.join(O,'review-sheet.jpg'),strip.toBuffer('image/jpeg',92));console.log('Rendered',audit.length,'frames');
})();
