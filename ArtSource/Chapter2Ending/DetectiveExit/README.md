...
현재 검토본은 `detective-exit-camera-v10.mp4`입니다. 배경 깊이에 따른 카메라 이동과 접지 기준의 보행을 함께 수정했습니다. 13.4초, 1080p/60fps, 무음이며 공통 960×540 격자에서 정수 2배 확대합니다. Unity 씬에는 아직 연결하지 않았습니다. 조사 자료·제작 방식·검수 한계는 `Research-and-Validation-v10.md`, 렌더와 검사는 `render_v10.py` / `validate_v10.py`를 참고하세요.
