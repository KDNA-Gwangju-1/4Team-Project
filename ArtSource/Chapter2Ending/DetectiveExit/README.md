...
최신 방향은 **빈 병원 복도와 창밖 나무의 바람 연출**입니다. `hospital-corridor-wind-v15.mp4`에는 걷는 캐릭터·문 동작·깜빡임·카메라 움직임이 없습니다. 창틀과 복도는 고정하고 창밖 수목 영역만 작게 흔들립니다. `Window-wind-detail-v15.mp4`는 움직임 확인용 창문 확대 영상입니다. 제작은 `render_v15.py`, 검사 내용은 `Validation-v15.md`를 참고하세요. Unity 연결 전 검토본입니다.

## 이전 v14 — 캐릭터 보행 방향 폐기

최신 검토본은 **`detective-exit-flicker-v14.mp4`**입니다. v13의 위치 전환·이동 블러에 맞춰 화면 밝기가 3프레임 동안 짧게 내려갔다 돌아오는 깜빡임을 추가했습니다. 걷는 구간에만 적용하며 완전히 검거나 흰 프레임은 넣지 않습니다. `Comparison-v13-v14.mp4`는 왼쪽 이전 / 오른쪽 수정 비교입니다. 제작은 `render_v14.py`, 상세는 `Validation-v14.md`를 참고하세요. 게임 연결 전 검토본입니다.
